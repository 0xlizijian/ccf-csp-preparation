## 202303-1 田地丈量
![image.png](https://cdn.nlark.com/yuque/0/2023/png/29271036/1693880449749-0a0e39ca-261a-4fec-ba53-5442b7c9e8f1.png#averageHue=%23faf8f7&clientId=uff349b05-5c2f-4&from=paste&height=414&id=ubde429fe&originHeight=518&originWidth=983&originalType=binary&ratio=1.25&rotation=0&showTitle=false&size=69812&status=done&style=none&taskId=ubcebb220-6edf-41d1-9f20-a9b5ec3f4e4&title=&width=786.4)
###### 样例输入
4 10 10
0 0 5 5
5 -2 15 3
...
###### 样例输出
44
![image.png](https://cdn.nlark.com/yuque/0/2023/png/29271036/1693880637409-a03be806-54ba-4e35-8378-73eed0211327.png#averageHue=%23f1f0f0&clientId=uff349b05-5c2f-4&from=paste&height=516&id=u19be2605&originHeight=645&originWidth=802&originalType=binary&ratio=1.25&rotation=0&showTitle=false&size=34057&status=done&style=none&taskId=u6864ad2d-e432-4195-9f27-6be7eb10e22&title=&width=641.6)
![image.png](https://cdn.nlark.com/yuque/0/2023/png/29271036/1693880661183-decd1a33-c92e-4f00-86bf-700e1c06be0a.png#averageHue=%23faf8f7&clientId=uff349b05-5c2f-4&from=paste&height=74&id=ud67dacfa&originHeight=92&originWidth=637&originalType=binary&ratio=1.25&rotation=0&showTitle=false&size=9452&status=done&style=none&taskId=u8e355f3f-7d05-4934-9012-9e084b66329&title=&width=509.6)
###### 思路
...
我们可以用二分查找寻找minX，左侧left = k, 右侧right = max($\sum_{i=1}^n t_i$)。对于每次查找，检查当开垦天数为x时，s是否小于等于m，我们可以找到minX
...
- 时间复杂度分析：$O(kn\sum_{i=1}^n cnt_i)$。整个程序的时间复杂度取决于calcStrStr函数，其中k是查询语句中表达式的个数，n是用户数，cnti为第i个用户具有的属性的个数。
- 空间复杂度分析：$O(\sum_{i=1}^n cnt_i)$。哈希表存储属性对应的用户需要$O(\sum_{i=1}^n cnt_i)$的空间。
##### 方法二
###### 思路
...
- 时间复杂度分析：$O(k*N/32)$。其中其中k是查询语句中条件的个数，N是bitset中二进制位的数量，bitset一次运算需要$O(N/32)$。
- 空间复杂度分析：$O(\sum_{i=1}^n cnt_i)$。哈希表存储属性对应的用户需要$O(\sum_{i=1}^n cnt_i)$的空间。
